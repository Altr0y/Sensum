...
export function MonitoringViewer({token, dashboardId, from, to, stationId}: MonitoringViewerProps) {
    ...
    const grafanaUrl =
        ...
        `?orgId=1&kiosk&theme=${resolvedTheme === "light" ? "light" : "dark"}&${timeRange}` +
        (stationId ? `&var-stationId=${stationId}` : "") +
    ...
    useEffect(() => {
        window.addEventListener("grafana-refresh", handleRefresh)
        return () => window.removeEventListener("grafana-refresh", handleRefresh)
    }, [handleRefresh])

    useEffect(() => {
        const id = DASHBOARDS.find((d) => d.id === dashboardId)?.id ?? DASHBOARDS[0].id
        setActiveId(id)
        setIframeState("loading")
    }, [dashboardId])
    ...
    return (
        <div className="-m-6 flex flex-col h-[calc(100vh-3.5rem)]">
            ...
            {tokenExpired && (
                <div
                    className="flex items-center gap-2 px-4 py-2 bg-yellow-900/30 border-b border-yellow-700/50 text-yellow-400 text-xs shrink-0">
